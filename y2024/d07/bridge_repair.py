...
class BridgeRepair:
    ...
    @staticmethod
    def get_sum_valid_targets(data: list[tuple[int, list[int]]]):
        return sum([target if BridgeRepair.is_valid(target, values) else 0 for target, values in data])

    @staticmethod
    def is_valid(target: int, values: list[int]):
    ...
    @staticmethod
    def get_sum_valid_targets_with_concat(data: list[tuple[int, list[int]]]):
        return sum([target if BridgeRepair.is_valid_with_concat(target, values) else 0
                    for target, values in data])

    @staticmethod
    def is_valid_with_concat(target: int, values: list[int]):
