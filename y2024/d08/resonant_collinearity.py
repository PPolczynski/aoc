...
def get_antinode(a:tuple[int, int], b:tuple[int, int]):
    y1, x1 = a
    y2, x2 = b
    distance = _get_distance(x1, y1, x2, y2)
    def get_coordinate(c1, c2, d):
        return c1 - ((d * (c1 - c2)) / distance)
    return round(get_coordinate(y1, y2, 2 * distance)), round(get_coordinate(x1, x2, 2 * distance))
    ...
    def get_antinodes_count(self) -> int:
        ...
        if _debug:
            for antinode in antinodes:
                self._city_map[antinode] = "#"
            print(self._city_map)
